...
def pinn_loss(y_pred, y_true, x):
    ...
    r = torch.sqrt(y_pred[:, 0]**2 + y_pred[:, 1]**2 + 1e-8)  # Add small epsilon to avoid division by zero
    ...
    PE = -G * M1 / r
    E_pred = KE + PE
    E_true = 0.5 * (y_true[:, 2]**2 + y_true[:, 3]**2) - G * M1 / torch.sqrt(y_true[:, 0]**2 + y_true[:, 1]**2 + 1e-8)
